...
BG, FG, CAPTION = (5, 7, 10), (127, 216, 255), (207, 233, 255)
...
class FaceRenderer:
    ...
    def frame(self, expression: str, mouth: float = 0.0, caption: str = "") -> np.ndarray:
        ...
        img = ImageChops.lighter(ImageChops.add(Image.new("RGB", (W, H), BG), glow), layer)

        d = ImageDraw.Draw(img)
        dim = lambda a: tuple(int(BG[i] + (FG[i] - BG[i]) * a) for i in range(3))
        if target.get("dots"):
            for i in range(3):
                a = 1.0 if int(self.t / 0.3) % 3 == i else 0.3
                cx, cy, r = W / 2 + u * 3.6 + i * u * 0.5, H * 0.2, u * 0.13
                d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=dim(a))
        if target.get("ring"):
            a, r = 0.35 + 0.25 * math.sin(self.t / 0.25), u * 3.7
            d.ellipse([W / 2 - r, H * 0.52 - r, W / 2 + r, H * 0.52 + r], outline=dim(a), width=max(1, int(u * 0.08)))
        if target.get("zzz"):
            a = 0.5 + 0.5 * math.sin(self.t / 0.6)
            d.text((W / 2 + u * 3.5, H * 0.25), "z", fill=dim(a), font=self.zfont, anchor="ls")
            d.text((W / 2 + u * 4.1, H * 0.17), "z", fill=dim(a), font=self.zfont, anchor="ls")
        if caption:
            lines = self._wrap(caption, W * 0.88)
            lh = self.font.size * 1.25
            y0 = H * 0.94 - lh * len(lines)
            for i, line in enumerate(lines):
                d.text((W / 2, y0 + i * lh), line, fill=CAPTION, font=self.font, anchor="ma")
